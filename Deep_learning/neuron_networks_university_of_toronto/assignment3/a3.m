...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  %which is obviously not the correct output. Your job is to replace that by a correct computation.
  ret.input_to_hid = model.input_to_hid * 0; %storing gradient with respect to W_xh
  ret.hid_to_class = model.hid_to_class * 0; %storing gradient with respect to W_ho (C,H)
  
  hid_input = model.input_to_hid * data.inputs; % input to the hidden units, i.e. before the logistic. size: <number of hidden units> by <number of data cases>
  hid_output = logistic(hid_input); % (H,N) output of the hidden units, i.e. after the logistic. size: <number of hidden units> by <number of data cases>
  class_input = model.hid_to_class * hid_output;
  
  class_normalizer = log_sum_exp_over_rows(class_input); % log(sum(exp of class_input)) is what we subtract to get properly normalized log class probabilities. size: <1> by <number of data cases>
  log_class_prob = class_input - repmat(class_normalizer, [size(class_input, 1), 1]); % log of probability of each class. size: <number of classes, i.e. 10> by <number of data cases>
  class_prob = exp(log_class_prob);
  N = size(class_prob, 2); %mini_batch_size
  dUpStream = 1.0/N * (class_prob - data.targets); %shape(C,N)
  ret.hid_to_class = dUpStream * hid_output' + wd_coefficient*model.hid_to_class;
  
  %backward gradient to hidden layer, 
  dhid_to_output = model.hid_to_class' * dUpStream;
  
  dhid_to_output = hid_output .* (1.0 - hid_output) .* dhid_to_output; %shape (H,N)
  
  ret.input_to_hid = dhid_to_output * data.inputs' + wd_coefficient*model.input_to_hid;
end
