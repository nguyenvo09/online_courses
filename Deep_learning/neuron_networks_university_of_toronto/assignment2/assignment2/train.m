...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    assert(isequal(size(embed_to_hid_weights_gradient), size(embed_to_hid_weights)));
    % Options:
    ...
    assert(isequal(size(hid_bias_gradient), size(hid_bias)));
    %disp(size(hid_bias_gradient));
    % Options
    ...
    %disp(size(back_propagated_deriv_2));
    ...
    if mod(m, show_validation_CE_after) == 0
      ...
      validationset_CE = validationset_CE + CE;
      count_validation_time = count_validation_time +1.0;
      if OctaveMode
        fflush(1);
      end
    end
  ...
  validationset_CE = validationset_CE/count_validation_time;
  count_validation_time = 0.0;
end
...
fprintf(1, '\r@Ben: Fial Average Vnalidation CE %.3f\n', validationset_CE);
