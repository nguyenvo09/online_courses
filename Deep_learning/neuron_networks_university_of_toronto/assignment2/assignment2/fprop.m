...
%     numhid1 is the dimensionality of the embedding space. (@ben's note: 
%     tao nghi la one-hot encoding cho first layer)
%
...
%@numwords here is the like a sentence of @numwords. batchsize is the
%number of sentences.
...
%this will be a tensor of shape (numhid1, numwords, batchsize) . Each
%matrix of shape (numhid1, numwords) is a matrix that represents for
%wordvectors of a sentence!!!!
embedding_layer_state = reshape(...
  word_embedding_weights(reshape(input_batch, 1, []),:)',...
  numhid1 * numwords, []);

assert(isequal(size(embedding_layer_state), [numhid1 * numwords, batchsize]));
%% COMPUTE STATE OF HIDDEN LAYER.
% Compute inputs to hidden units.
T = embed_to_hid_weights' * embedding_layer_state;
assert(isequal(size(T), [numhid2, batchsize]));
%inputs_to_hidden_units = embed_to_hid_weights' * embedding_layer_state + ...
%  repmat(hid_bias, 1, batchsize);

inputs_to_hidden_units = bsxfun(@plus, T, hid_bias);
assert(isequal(size(inputs_to_hidden_units), size(T)));
...
inputs_to_softmax = bsxfun(@plus, hidden_layer_state'*hid_to_output_weights, output_bias');
assert(isequal(size(inputs_to_softmax), [batchsize, vocab_size]));
% Options
...
inputs_to_softmax = bsxfun(@minus, inputs_to_softmax, max(inputs_to_softmax, [], 2));
...
output_layer_state = bsxfun(@rdivide, output_layer_state, sum(output_layer_state, 2));
