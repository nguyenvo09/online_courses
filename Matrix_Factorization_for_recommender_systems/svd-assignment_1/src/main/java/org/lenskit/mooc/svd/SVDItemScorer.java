...
public class SVDItemScorer extends AbstractItemScorer {
    ...
    public ResultMap scoreWithDetails(long user, @Nonnull Collection<Long> items) {
        ...
        while(it.hasNext()){
            long itemId = it.next();
            RealVector itemVec = model.getItemVector(itemId);
            if(itemVec == null){
                continue;
            }
            RealVector res = userFeatures.ebeMultiply(itemVec).ebeMultiply(featureWeights);
            double vl = 0.0;
            for(int i=0; i<res.getDimension(); i+=1){
                vl += res.getEntry(i);
            }

            // TODO Add the predicted offsets to the baseline score
            vl += baseline.getIntercept()+
                    baseline.getUserBias(user)+
                    baseline.getItemBias(itemId);
